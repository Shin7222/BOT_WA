...
//       - ada  → pairing code ditampilkan saat 'connecting'
//       - tidak → QR code
// ─────────────────────────────────────────────────────
async function startConnection(onReady, onMessage, opts = {}) {
  ...
  if (!hasSession) {
    if (usePairing) {
      ...
      console.log(chalk.white("   Menghubungkan ke WhatsApp...\n"));
    } else {
      console.log(chalk.green("\n📱 Login via QR Code"));
      console.log(
        chalk.white(
  ...
  sock.ev.on(
    ...
    async ({ connection, lastDisconnect, qr }) => {
      ...
      // PERBAIKAN: dipanggil di sini, bukan di 'open'
      if (connection === "connecting" && usePairing && !pairingRequested) {
        pairingRequested = true;

        // Delay kecil agar socket siap menerima request
        setTimeout(async () => {
          try {
            ...
            console.log(
              chalk.cyan(
                `│  🔑 Kode Pairing: ${chalk.bold.yellow(String(formatted).padEnd(15))}│`,
              ),
            );
            console.log(chalk.cyan("└──────────────────────────────────┘"));
            ...
            console.log(
              chalk.white(
                "   HP → Perangkat Tertaut → Tautkan dengan Nomor Telepon\n",
              ),
            );
          } catch (err) {
      ...
      if (connection === "open") {
        ...
        console.log(
          chalk.white(`   Nama  : ${chalk.bold(sock.user?.name ?? "-")}`),
        );
        console.log(
          chalk.white(`   Waktu : ${new Date().toLocaleString("id-ID")}\n`),
        );

        if (opts.retryCount > 0) logger.info("Reconnect berhasil!");
        ...
        switch (code) {
          ...
          default:
            if (retryCount < MAX_RETRIES) {
              ...
              logger.info(
                `Reconnect ${retryCount + 1}/${MAX_RETRIES} dalam ${delay / 1000}s...`,
              );
              setTimeout(
                () =>
                  startConnection(onReady, onMessage, {
